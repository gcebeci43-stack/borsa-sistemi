...
# --- 2. LEMNISCATE ANALİZ MOTORU (13 KRİTER) ---
...
def piyasayi_analiz_et():
    # Buraya orijinal listeni yapıştırabilirsin
    tickers = [
    ...
    with st.status("♾️ Engine: Piyasalar taranıyor...", expanded=True) as status:
...
if menu_secim == "📊 Canlı Terminal":
    ...
    if st.session_state.sorgulanan_hisse:
        ...
        if not res_df.empty:
            ...
            z_secici = st.radio("Zaman Aralığı:", ["1 Hafta", "1 Ay", "6 Ay", "1 Yıl"], horizontal=True)
        ...
        else: st.info("Analiz verisi yok.")
    ...
    if not perf_df.empty:
        ...
        for i, (idx, row) in enumerate(perf_df.iterrows()):
            with cols[i % 2].container(border=True):
                ...
                # Eğer guncel_fiyat henüz Update edilmemişse başlangıç fiyatını göster
        ...
        st.warning("Bu hafta için mühürlenmiş liste yok. Analiz yapın.")
    conn.close()
