...
def piyasayi_analiz_et():
    # Liste paylaştığın tam liste olarak kullanılmalı
    tickers = [
    ...
    # --- TÜRKİYE ZAMAN DİLİMİ AYARI ---
    tr_tz = pytz.timezone('Europe/Istanbul')
    simdi = datetime.now(tr_tz)
    tarih_str = simdi.strftime('%Y-%m-%d %H:%M')
    hafta_no = simdi.isocalendar()[1]
    
    with st.status("♾️ Lemniscate Engine: Analiz Başlatıldı...", expanded=True) as status:
        ...
        if all_results:
            ...
            # HAFTALIK SABİTLEME: Kayıt yoksa Top 10'u dondur
            ...
            conn.commit()
            status.update(label=f"✅ Analiz Tamamlandı!", state="complete")
    conn.close()
...
if menu_secim == "📊 Canlı Terminal":
    ...
    if st.session_state.sorgulanan_hisse:
        ...
        else: st.info("Henüz analiz verisi yok.")

elif menu_secim == "📈 Haftalık Performans":
    st.title("📈 Haftalık Performans Takibi")
    tr_tz = pytz.timezone('Europe/Istanbul')
    hafta_no = datetime.now(tr_tz).isocalendar()[1]
    st.info(f"📅 Şu an {hafta_no}. haftadasınız. Liste her Pazartesi sabahı ilk analizde mühürlenir.")
    ...
    if not perf_df.empty:
        ...
        for i, (idx, row) in enumerate(perf_df.iterrows()):
            with cols[i % 2].container(border=True):
                ...
                st.caption(f"Kayıt: {row['kayit_tarihi']}")
    else: st.warning("Bu hafta için henüz mühürlü liste yok. Lütfen terminalden analiz başlatın.")
    conn.close()
