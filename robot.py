...
# --- 2. LEMNISCATE ANALİZ MOTORU (13 İNDİKATÖR) ---
def analyze_stock(data):
    """Hisse verisini 13 farklı teknik kritere göre puanlar."""
    if len(data) < 50: return 0.0, 0.0
    score = 0.0
    try:
        # Veri yapısını düzelt (MultiIndex hatasını önler)
        ...
        # 11. ATR Tabanlı Stop Loss (Teknik Stop)
...
def robot_analiz():
    ...
    print("-" * 30)
    print(f"🚀 Analiz Başlatıldı: {tarih_str}")
    ...
    try:
        ...
        if all_results:
            ...
            ''', all_results)
            conn.commit()
            print(f"✅ Başarılı: {len(all_results)} analiz veritabanına mühürlendi.")
...
# --- ZAMANLAYICI MANTIĞI ---
# Her gün sabah 09:30'da çalıştır
schedule.every().day.at("09:30").do(robot_analiz)

print("♾️ Lemniscate Robot Nöbette...")
print("🤖 Görev: Her sabah 09:30'da analiz yapılacak.")
print("💡 İpucu: Robotun çalışması için bu terminal penceresini kapatmayın.")

# Deneme yapmak istersen bu satırı aktif edebilirsin (Hemen çalıştırır):
#robot_analiz()
